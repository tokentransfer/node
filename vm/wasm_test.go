...
func (suite *WasmSuite) TestData(c *C) {
	...
	c.Assert(err, IsNil)
	_, o, err := core.UnmarshalData(hexData)
	c.Assert(err, IsNil)
...
func (suite *WasmSuite) TestList(c *C) {
	...
	listData, err := hex.DecodeString(string(""))
	c.Assert(err, IsNil)
	_, o, err := core.UnmarshalData(listData)
	c.Assert(err, IsNil)
	util.PrintJSON("list", o)
}
	...
	for index, testCase := range cases {
		...
		for i := 0; i < loop; i++ {
			...
			if len(newWasmData) > 0 {
				...
				m := msg.(*pb.DataMap)
				c.Assert(m, NotNil)
				_, o, err := core.UnmarshalData(newData)
				c.Assert(err, IsNil)
				util.PrintJSON("wasm data", o)
			}
